...
def run_loop(model: str, topic: str, iterations: int, think: bool,
    ...
    for i in range(1, iterations + 1):
        ...
        prompt = (
            ...
            f"[HAL_QUESTIONS]\n"
            f"1. ...\n\n"
            f"Output budget (strict — total output must fit in 1500 tokens):\n"
            f"- [PROPOSAL] <= 800 characters\n"
            f"- [UNCERTAINTY] <= 600 characters\n"
            f"- [REQUIRED_RECEIPTS] = 3 bullets max\n"
            f"- [HAL_QUESTIONS] = 3 questions max\n\n"
            f"Forbidden:\n"
            f"- Do NOT output [PROCESS OF THOUGHT].\n"
            f"- Do NOT output analysis sections.\n"
            f"- Do NOT output doctrine amendments.\n"
            f"- Do NOT output process notes.\n"
            f"- Do NOT output halted testing scope or halt conditions.\n"
